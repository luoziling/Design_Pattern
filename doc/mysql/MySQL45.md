...
- 通过redo log实现持久化（crash-safe） undolog  实现可重复读，事务回滚 以将数据恢复到合适的状态
- 长事务
  - 导致redo log过长占用空间过多
  - 占用锁资源
- 事务启动
...
- 幻读指RR级别下insert产生的问题
- RR级别通过undo log解决快照读幻读问题
- RR级别通过间隙锁解决RR级别当前读的update幻读问题



**幻读的定义**

- RR级别下快照读看不到其他事务的插入数据，幻读是RR级别的当前读（in share mode/for update）
- 幻读仅指insert 不管update

**幻读的危害**

- 语义不一致
  - 例如存在事务A/B 事务A锁定x = 5 事务B把另一条数据的x改成5 导致A的锁定语义没有充分体现
- 数据不一致
  - 由于事务提交才真正执行语句，若事务A修改x=5的id =0，其他事务提前提交并且把x修改为5 则产生把其他事务修改后的数据也改为特定值的问题
- **也就是说，即使把所有的记录都加上锁，还是阻止不了新插入的记录**，这也是为什么“幻读”会被单独拿出来解决的原因。
...
- 只有串行能彻底解决幻读问题 但是性能不太行
- 间隙锁加锁也是有范围的，超出范围的是否就还有幻读，不会，间隙锁锁定的就是特定值范围
